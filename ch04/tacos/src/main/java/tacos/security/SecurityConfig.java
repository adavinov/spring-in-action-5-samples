...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
     ...
     * auth .jdbcAuthentication() .dataSource(dataSource) .usersByUsernameQuery(
     * "select username, password, enabled from Users " + "where username=?") .authoritiesByUsernameQuery(
     * "select username, authority from UserAuthorities " + "where username=?") .passwordEncoder(new
     * StandardPasswordEncoder("53cr3t");
     *
     ...
     *
     * @Override protected void configure(AuthenticationManagerBuilder auth) throws Exception { auth .ldapAuthentication()
     * .userSearchFilter("(uid={0})") .groupSearchFilter("member={0}"); } //end::configureAuthentication_ldap[]
     */
